...
int main (int argc, char **argv)
{
 ...
 for ( ; ; ) {
  ...
  while ( (n = recv(connfd, buf, MAXLINE,0)) > 0)  {
   ...
   //PULL OLD FILE, Requires Filename
   if (strcmp("PUO",token)==0)  {
	...
	//PULL NEW FILE, snaps photo and sends file, doesn't require filename
	//Filename based on timestamp when taking the photo
   if (strcmp("PUN",token)==0)  {
	char port[MAXLINE],buffer[MAXLINE],char_num_blks[MAXLINE],char_num_last_blk[MAXLINE];
	int datasock,lSize,num_blks,num_last_blk,i;
	FILE *fp;
	//token=strtok(NULL," \n");
	...
	//Take photo and convert to lossy JPEG
